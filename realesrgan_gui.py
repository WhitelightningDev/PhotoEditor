...
class RealESRGANApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Real-ESRGAN Image Processor")
        self.root.geometry("900x500")  # Window size
        self.root.config(bg="#ececec")  # Background color
        ...
        self.title_frame = Frame(root, bg="#4CAF50")
        self.title_frame.pack(fill='x')

        self.title_label = Label(self.title_frame, text="Real-ESRGAN Image Processor", bg="#4CAF50", fg="white", font=("Arial", 20, "bold"))
        self.title_label.pack(pady=15)

        # Create a main frame for better organization
        self.frame = Frame(root, bg="#ececec")
        self.frame.pack(pady=20, padx=20, fill='both', expand=True)

        # Input and Output Frame
        self.io_frame = Frame(self.frame, bg="#ececec")
        self.io_frame.pack(pady=10, fill='x')

        # Input file label and entry
        self.label_input = Label(self.io_frame, text="Input File:", bg="#ececec", font=("Arial", 12))
        self.label_input.grid(row=0, column=0, padx=10, pady=10, sticky='w')

        self.label_input_file = Label(self.io_frame, textvariable=self.input_file, bg="#ececec", font=("Arial", 10), fg="#555")
        self.label_input_file.grid(row=0, column=1, padx=10, pady=10, sticky='w')

        # Output file label
        self.label_output = Label(self.io_frame, text="Output File:", bg="#ececec", font=("Arial", 12))
        self.label_output.grid(row=1, column=0, padx=10, pady=10, sticky='w')

        self.label_output_file = Label(self.io_frame, textvariable=self.output_file, bg="#ececec", font=("Arial", 10), fg="#555")
        self.label_output_file.grid(row=1, column=1, padx=10, pady=10, sticky='w')

        # Create a button frame
        self.button_frame = Frame(self.frame, bg="#ececec")
        self.button_frame.pack(pady=20)

        # Load button
        self.button_load = Button(self.button_frame, text="Load Image", command=self.load_image, bg="#2196F3", fg="white", font=("Arial", 12, "bold"), relief="flat", padx=10)
        self.button_load.grid(row=0, column=0, padx=10, pady=10, sticky='ew')

        # Process button
        self.button_process = Button(self.button_frame, text="Process Image", command=self.start_processing, bg="#FF9800", fg="white", font=("Arial", 12, "bold"), relief="flat", padx=10)
        ...
        # Status bar
        self.status_bar = Label(root, text="Status: Ready", bg="#ececec", font=("Arial", 10))
        self.status_bar.pack(side='bottom', fill='x')

        # Dark mode toggle button at the bottom right
        self.button_dark_mode = Button(root, text="Toggle Dark Mode", command=self.toggle_dark_mode, bg="#9C27B0", fg="white", font=("Arial", 12, "bold"), relief="flat", padx=10)
    ...
    def process_image(self, output_path):
        ...
        if input_path:
            ...
            command = f'"C:\\Users\\DellUser\\IdeaProjects\\PhotoEditor\\realesrgan\\realesrgan-ncnn-vulkan.exe" -i "{input_path}" -o "{output_path}"'
    ...
    def toggle_dark_mode(self):
        ...
        bg_color = "#333333" if self.dark_mode else "#ececec"
        fg_color = "white" if self.dark_mode else "#555"
        title_bg_color = "#4CAF50" if not self.dark_mode else "#2E7D32"
        ...
        for widget in self.button_frame.winfo_children():
            widget.config(bg="#9C27B0" if self.dark_mode else "#FF9800", fg="white")
